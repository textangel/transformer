...
from .vocab_preprocess import read_corpus, pad_sents
...
class VocabEntry(object):
    ...
    def to_input_tensor(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        """
        Modified from CS224N code because I also need to return the masks.
        """
...
class Vocab(object):
    ...
    def __init__(self, src_vocab: VocabEntry, tgt_vocab: VocabEntry):
        ...
        self.src = src_vocab
        self.tgt = tgt_vocab
        assert self.src.word2id['<pad>'] == self.tgt.word2id['<pad>']
...
def text_to_tensor(source: List[List[str]], target: List[List[str]], vocab, device) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    source_padded = vocab.src.to_input_tensor(source, device=device)   # Tensor: (src_len, b)
    target_padded = vocab.tgt.to_input_tensor(target, device=device)   # Tensor: (tgt_len, b)
